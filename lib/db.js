...
// Open a fresh client per query. Over Supabase's transaction pooler a
// long-lived Pool can hang (the pooler assigns a different backend per
// statement, which pg's pool does not tolerate). A single Client per
// operation is the recommended serverless pattern and avoids the stall.
async function query(text, params = []) {
  const client = new Client({
    connectionString: pgUrl,
    ssl: pgUrl.includes('supabase') || pgUrl.includes('pooler') ? { rejectUnauthorized: false } : undefined,
...
async function pgEnsureTable(collection) {
  ...
  `);
  await query(`CREATE INDEX IF NOT EXISTS "${safeName}_data_idx" ON "${safeName}" USING GIN (data)`);
}
...
async function getAll(collection) {
  ...
  const res = await query(`SELECT data FROM "${collection}"`);
...
async function find(collection, predicateFn) {
  ...
  await pgEnsureTable(collection);
  const all = await query(`SELECT data FROM "${collection}"`);
...
async function update(collection, id, patch) {
  ...
  await pgEnsureTable(collection);
  const existing = await query(`SELECT data FROM "${collection}" WHERE id = $1`, [id]);
...
async function remove(collection, id) {
  ...
  await pgEnsureTable(collection);
  const res = await query(`DELETE FROM "${collection}" WHERE id = $1`, [id]);
...
async function removeWhere(collection, predicateFn) {
  ...
  await pgEnsureTable(collection);
  const all = await query(`SELECT id, data FROM "${collection}"`);
  const toRemove = all.rows.filter(r => predicateFn(r.data));
  if (!toRemove.length) return 0;
  await query(`DELETE FROM "${collection}" WHERE id = ANY($1)`, [toRemove.map(r => r.id)]);
...
async function seedIfEmpty() {
  ...
  for (const item of defaults) {
    ...
    if (usePostgres) {
      ...
      if (checks.length) {
        const res = await query(`SELECT id FROM "${safeName}" WHERE ${checks.join(' OR ')} LIMIT 1`, params);
...
async function migrateFromJson() {
  ...
  for (const name of collections) {
    ...
    try {
      ...
      const existing = await query(`SELECT COUNT(*) FROM "${name}"`);
      if (Number(existing.rows[0].count) > 0) continue; // already migrated
      for (const rec of arr) {
        const id = rec.id || genId();
        await query(`INSERT INTO "${name}" (id, data, created_at, updated_at) VALUES ($1, $2, $3, $4) ON CONFLICT (id) DO NOTHING`,
